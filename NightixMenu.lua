...
return function(guilibrary, OptionFunctions, connections, userInputService, tweenService, textService, mouse, spawn)
    ...
    local function createTextList(container, argstable, toggleName, tabName)
        ...
        function api:CreateListObject(value)
            ...
            local removeWindow = itemLabel:AddOption(1) -- gear: remove entry
            removeWindow:AddButton({
                Name = "Remove",
                Icon = "close",
                Callback = function()
    ...
    -- ------------------------------------------------------------------
    -- UserSettings (bottom-left settings popup)
    -- ------------------------------------------------------------------
    do
        ...
        settings:AddButton({
            ...
            Callback = function()
                if pcall(function() return setclipboard end) then
                    setclipboard("https://discord.gg/nightix")
                end
            end,
        })
    end
